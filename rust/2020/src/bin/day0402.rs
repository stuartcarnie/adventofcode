...
impl FromStr for Passport {
    ...
    fn from_str(rec: &str) -> Result<Self, Self::Err> {
        // parse into (key, val) pairs
        let fields: Vec<_> = rec.split_ascii_whitespace()
            .map(|x| {
                let kv = KEY_VAL.captures(x).unwrap();
                let k = kv.get(1).unwrap().as_str();
                let v = kv.get(2).unwrap().as_str();
                (k, v)
            })
            .collect();

        if !fields.iter().all(|x| REQUIRED_KEYS.contains(x.0)) {
            return Err(format!("missing required keys"));
        }
        ...
        for (key, val) in fields {
            match key {
                "byr" => {
                    p.byr = Some(val.parse::<i32>().map_err(|e| e.to_string())?)
                }
                _ => {}
            }
        }
...
lazy_static! {
    static ref KEY_VAL: Regex = Regex::new(r"(\w+):(\S+)").unwrap();
    static ref REQUIRED_KEYS: HashSet<&'static str> = vec!["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"].into_iter().collect();
}

fn main() {
    let required_keys: HashSet<&str> = vec!["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"].into_iter().collect();
    let record = Regex::new(r"\n\n").unwrap();
    let key_val = Regex::new(r"(\w+):(\S+)").unwrap();
    let res = record.split(INPUT)
        .filter_map(|rec| {
            Passport::from_str(rec).ok()
        })
        .count();
    println!("{}", res);
}

static INPUT: &str = r#"ecl:amb
pid:690616023
