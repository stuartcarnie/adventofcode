...
public struct Day02 {
    ...
    static let regex = try! NSRegularExpression(pattern: #"^(\d+)-(\d+) (.): (.*)$"#)
    
    public static func parsed() -> [(Int, Int, Character, String)] {
        input.compactMap({ a -> (Int, Int, Character, String)? in
            guard
                let cg = regex.matches(in: a, options: [], range: NSRange(a.startIndex..<a.endIndex, in: a)).first
            else { return nil }
            
            if cg.numberOfRanges == 5,
               let r1 = Range(cg.range(at: 1), in: a), let min = Int(a[r1]),
               let r2 = Range(cg.range(at: 2), in: a), let max = Int(a[r2]),
               let r3 = Range(cg.range(at: 3), in: a),
               let r4 = Range(cg.range(at: 4), in: a)
            {
                return (min, max, Character(String(a[r3])), String(a[r4]))
            }
