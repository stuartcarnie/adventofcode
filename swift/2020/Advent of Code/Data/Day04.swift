...
public struct Day04 {
    ...
    public static func records() -> [String] {
        let ranges = input.ranges(ofRegex: "^\n", options: [.anchorsMatchLines])
        var start  = input.startIndex
        var res = [String]()
        for r in ranges {
            let end = r.lowerBound
            res.append(input[start..<end].replacingOccurrences(of: "\n", with: " ").trimmingCharacters(in: .whitespacesAndNewlines))
            start = r.upperBound
        }
        res.append(input[start..<input.endIndex].replacingOccurrences(of: "\n", with: " ").trimmingCharacters(in: .whitespacesAndNewlines))
        return res
    }
}
